...
class Reviewer(BaseModel):
    ...
    is_group: Optional[bool] = Field(
        default=None,
        description="True if target is a group/project, False if user, None if unknown",
    )
    github_username: Optional[str] = Field(default=None, description="Resolved GitHub username")
    github_user_id: Optional[int] = Field(
        default=None, description="Resolved GitHub numeric user ID"
    )
...
class UnresolvedUser(BaseModel):
    ...
    reason: str = Field(
        ..., description="Why resolution failed: 'not_found', 'no_github_linked', or error message"
    )
    referenced_in: List[str] = Field(
        default_factory=list, description="Rule IDs that reference this user"
    )
...
class ScrapeStatus(BaseModel):
    ...
    github_complete: bool = Field(
        default=False, description="Whether GitHub resolution is complete"
    )
...
class Metadata(BaseModel):
    ...
    total_rules: int
    total_groups: int
    total_users_resolved: int = Field(
        default=0, description="Number of users with GitHub usernames resolved"
    )
    total_users_unresolved: int = Field(
        default=0, description="Number of users without GitHub usernames"
    )
    phabricator_instance: str
    scrape_status: Optional[ScrapeStatus] = Field(
        default=None, description="Completion status for resumable scraping"
    )
